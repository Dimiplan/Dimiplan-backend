...
const addAdminField = async () => {
    try {
        ...
        const hasAdminField = columns[0].some(
            (column) => column.Field === "isAdmin",
        );
        ...
        // isAdmin 필드 추가
        await db.raw(
            "ALTER TABLE users ADD COLUMN isAdmin TINYINT(1) NOT NULL DEFAULT 0 COMMENT '관리자 권한 (0: 일반사용자, 1: 관리자)'",
        );
